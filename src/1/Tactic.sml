...
(* intersect_sorted : ('a * 'a -> order) -> 'a list * 'a list -> 'a list
  get the intersection of two lists sorted according to cmp *)
fun intersect_sorted cmp (l, []) = []
  | intersect_sorted cmp ([], l) = []
  | intersect_sorted cmp (h1 :: t1, h2 :: t2) =
    case cmp (h1, h2) of EQUAL => h1 :: intersect_sorted cmp (t1, t2)
      | LESS => intersect_sorted cmp (t1, h2 :: t2)
      | GREATER => intersect_sorted cmp (h1 :: t1, t2) ;

(* find_common_subterms : term -> term -> term list
  split up lhs and rhs into subterms joined by opr, find subterms on both sides
  eg find_common_subterms (``$+``) (``a + (b + c) = d + b + a``)
  gives [``a``, ``b``] *)
fun find_common_subterms opr eqtm =
  let val (lhs, rhs) = dest_eq eqtm ;
    val lhs_tms = strip_binop (total (dest_binop opr Bind)) lhs ;
    val rhs_tms = strip_binop (total (dest_binop opr Bind)) rhs ;
    val s_lhs_tms = Listsort.sort Term.compare lhs_tms ;
    val s_rhs_tms = Listsort.sort Term.compare rhs_tms ;
  in intersect_sorted Term.compare (s_lhs_tms, s_rhs_tms) end ;

(*-----------------------------------------------------------------------*
 * AC_TAC - simplify goal using associative + commutative laws           *
 *                                                                       *
 * The tactic is given an associative and commutative law (it deduces    *
 * the relevant operator and type from these) in the form of the inbuilt *
 * ones, and a goal equation to prove. It will try to prove this.        *
 * Example:                                                              *
 *  AC_TAC (ADD_ASSOC,ADD_SYM)
      ([], ``(1 + 5) + (2 + 4) = 4 + (5 + (3 + 1))``)                    *
    gives new goal ([], ``2 = 3``)                                       *
 * [JED 15.08.13]                                                        *
 *-----------------------------------------------------------------------*)

fun AC_IMP_THM fns [] eqtm = ASSUME eqtm
  | AC_IMP_THM (fns as {opr, move_subt, ...}) (ct :: cts) eqtm =
    let val (lhs, rhs) = dest_eq eqtm ;
      val SOME (rest_lhs, bubble_lhs) = move_subt ct lhs ;
      val SOME (rest_rhs, bubble_rhs) = move_subt ct rhs ;
      val red_eq = mk_eq (rest_lhs, rest_rhs) ;
      val red_th = ASSUME red_eq ;
      val bub_th = AP_TERM (mk_comb (opr, ct)) red_th ;
      val th1 = TRANS bubble_lhs bub_th ;
      val th2 = TRANS th1 (SYM bubble_rhs) ;
      val cts_ths = AC_IMP_THM fns cts red_eq ;
    in PROVE_HYP cts_ths th2 end
    (* as move_subt requires tm opr tm *)
    handle _ =>
      let val (lhs, rhs) = dest_eq eqtm ;
      in if lhs = rhs then REFL lhs else ASSUME eqtm end
      handle _ => ASSUME eqtm ;

(* AC_TAC : thm * thm -> tactic *)
fun AC_TAC (assoc, comm) =
  let val fns as {opr, ...} = Conv.ac_fns (assoc, comm) ;
    fun tac (asl, w) = 
      let
	val cts = find_common_subterms opr w ;
	val ac_imp_thm = AC_IMP_THM fns cts w ;
      in prim_irule ac_imp_thm (asl, w) end ;
  in tac end ;
